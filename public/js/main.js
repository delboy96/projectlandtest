document.addEventListener("DOMContentLoaded", function(event) {
    ...
    function NewAnswer(questionId){
      return `<div class="answerdiv text-right w-full" data-question-id="${questionId}">
                    <input type="text" class=" w-full lg:w-1/2 py-2 px-3 bg-gray-50 text-gray-700 rounded-xl border border-gray-400 focus:outline-none focus:border-teal-600" placeholder="Ponuđeni odgovor" required>
               </div>`
    };
    ...
    function NewQuestion(questionId){
        return `<div id="question" class="question flex flex-col text-gray-700 space-y-6 lg:space-y-4">
                        <div class="flex flex-col lg:flex-row lg:grid grid-cols-3 gap-2">
                            ...
                            <input id="question" type="text" data-question-id="${questionId}" class="col-span-2 py-2 px-3 bg-gray-50 text-gray-700 rounded-xl border border-gray-400 focus:outline-none focus:border-teal-600" placeholder="Tekst pitanja *" required>
                        </div>
    ...
    forma.addEventListener('click', function (e) {
        if (hasClass(e.target.closest('button'), 'addansw')) {
            ...
            // proba closest
            // let answersDiv = e.target.parentElement.parentElement.parentElement.parentElement.previousElementSibling;
    ...
    createbtn.addEventListener('click', function (e){
        ...
        let questiond = document.querySelectorAll('.questiondiv');
        let answersd = document.querySelectorAll('.answerdiv');
        console.log(questiond);
        console.log(answersd);

        //za jedno pitanje uzmem id, za taj id nadjem sve divove sa tim id (for),pushujem u niz odgovora jednog pitanja

        const data = { title, deadline, question, answers };

        // console.log(answers)

        // TODO: get all answer class inputs for right answers ;
        // TODO: solve cannot read property of null on form click(because of addansw btn) ;
